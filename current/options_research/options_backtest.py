...
class CostModel:
    ...
    def exit_friction(self) -> float:
        return self.commission_per_leg * 2
...
def construct_trade(
    ...
    if kind == "naked":
        ...
        closest = _nearest_prior_bar(leg_mins, target_min)
        if closest is None:
            return None
        ...
        s_closest = _nearest_prior_bar(short_mins, target_min)
        l_closest = _nearest_prior_bar(long_mins, target_min)
...
def evaluate_day_overnight(
    ...
    for idx in range(ctx["n"]):
        ...
        if tk == "naked":
            entry_cost = cost.slippage_per_leg * 100 + cost.commission_per_leg
            exit_cost = cost.commission_per_leg
        else:
