...
=============================
Reads results/options_wf/regime_db.jsonl and answers the core question:
"Is there consistent edge across windows, or are results concentrated
in a few lucky windows?"

Usage:
    python current/options_wf/wf_analyze.py
    python current/options_wf/wf_analyze.py --file results/options_wf/regime_db.jsonl
"""
...
def analyze(records: list[dict]) -> None:
    ...
    # ── By archetype ─────────────────────────────────────────────────────
    print(f"\n{'─' * 72}")
    print("FORWARD PROFITABLE RATE BY ARCHETYPE")
    print(f"{'─' * 72}")
    ...
    for arch, recs in sorted(by_arch.items()):
        ...
        arch_rows.append((arch, total, pos, pos / total * 100 if total else 0, pnl))

    arch_rows.sort(key=lambda x: -x[3])
    print(f"  {'Archetype':<25s} {'Count':>6s} {'Pos':>5s} {'Rate':>7s} {'CumPnL':>10s}")
    for arch, total, pos, rate_a, pnl in arch_rows:
        print(f"  {arch:<25s} {total:>6d} {pos:>5d} {rate_a:>6.1f}% ${pnl:>9,.0f}")

    # ── By direction ─────────────────────────────────────────────────────
    print(f"\n{'─' * 72}")
    print("FORWARD PROFITABLE RATE BY DIRECTION")
    print(f"{'─' * 72}")
    ...
    for d, recs in sorted(by_dir.items()):
        ...
        pos = sum(1 for r in recs if r.get("forward_profitable", False))
        pnl = sum(r.get("val_cum_pnl", 0) for r in recs)
        print(f"  {d:<10s} {total:>5d} regimes | {pos:>4d} profitable "
              f"({pos/total*100:.1f}%) | ${pnl:>+10,.0f} cum PnL")

    # ── By trade type ────────────────────────────────────────────────────
    print(f"\n{'─' * 72}")
    print("FORWARD PROFITABLE RATE BY TRADE TYPE")
    print(f"{'─' * 72}")
    ...
    for r in records:
        ...
        kind = tt.get("kind", "?")
        if kind == "naked":
            label = f"naked_{tt.get('moneyness', '?')}"
        else:
            label = f"{kind}_w{tt.get('width', '?')}"
        by_tt[label].append(r)

    tt_rows = []
    for label, recs in sorted(by_tt.items()):
        total = len(recs)
        pos = sum(1 for r in recs if r.get("forward_profitable", False))
        pnl = sum(r.get("val_cum_pnl", 0) for r in recs)
        tt_rows.append((label, total, pos, pos / total * 100 if total else 0, pnl))

    tt_rows.sort(key=lambda x: -x[3])
    print(f"  {'TradeType':<22s} {'Count':>6s} {'Pos':>5s} {'Rate':>7s} {'CumPnL':>10s}")
    for label, total, pos, rate_tt, pnl in tt_rows:
...
def main():
    ...
    parser.add_argument("--file", type=str,
                        default="results/options_wf/regime_db.jsonl")
